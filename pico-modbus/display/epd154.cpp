...
void epd154::DEV_Digital_Write(uint8_t pin, bool val) {
    gpio_put(pin, val);
};
bool epd154::DEV_Digital_Read(uint8_t pin) {
    return gpio_get(pin);
};
void epd154::DEV_SPI_WriteByte(uint8_t val) {
    uint8_t *tmp = &val;
    spi_write_blocking(PICO_DEFAULT_SPI_INSTANCE, tmp, 1);
    while (spi_is_busy(PICO_DEFAULT_SPI_INSTANCE));

};
void epd154::DEV_Delay_ms(uint32_t ms) {
    sleep_ms(ms);
};
int epd154::DEV_Module_Init(void){
    ...
    DEV_Digital_Write(EPD_CS_PIN, true);
    DEV_Delay_ms(100);
...
void epd154::DEV_Module_Exit(void) {
    DEV_Digital_Write(EPD_DC_PIN, 0);
    DEV_Digital_Write(EPD_CS_PIN, 0);
...
parameter:
******************************************************************************/
void epd154::EPD_1IN54_V2_Reset(void)
{
    DEV_Digital_Write(EPD_RST_PIN, 1);
    DEV_Delay_ms(200);
    DEV_Digital_Write(EPD_RST_PIN, 0);
...
******************************************************************************/
void epd154::EPD_1IN54_V2_SendCommand(UBYTE Reg)
{
    DEV_Digital_Write(EPD_DC_PIN, 0);
    DEV_Digital_Write(EPD_CS_PIN, 0);
...
parameter:
    Data : Write data
******************************************************************************/
void epd154::EPD_1IN54_V2_SendData(UBYTE Data)
{
...
parameter:
******************************************************************************/
void epd154::EPD_1IN54_V2_ReadBusy(void)
{
    Debug("e-Paper busy\r\n");
...
parameter:
******************************************************************************/
void epd154::EPD_1IN54_V2_TurnOnDisplay(void)
{
...
parameter:
******************************************************************************/
void epd154::EPD_1IN54_V2_TurnOnDisplayPart(void)
{
    EPD_1IN54_V2_SendCommand(0x22);
...
void epd154::EPD_1IN54_V2_Lut(UBYTE *lut)
{
...
void epd154::EPD_1IN54_V2_SetLut(UBYTE *lut)
{
...
void epd154::EPD_1IN54_V2_SetWindows(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend)
{
...
void epd154::EPD_1IN54_V2_SetCursor(UWORD Xstart, UWORD Ystart)
{
...
epd154::epd154() : mono_horiz(200, 200) {
    DEV_Module_Init();
    EPD_1IN54_V2_Init();
...
void epd154::show() {
    ...
    for(uint32_t i = 0; i < size; ++i) {
        EPD_1IN54_V2_SendData(buf[i]);
    }
    EPD_1IN54_V2_TurnOnDisplay();
}


void epd154::EPD_1IN54_V2_Init(void)
{
    EPD_1IN54_V2_Reset();
