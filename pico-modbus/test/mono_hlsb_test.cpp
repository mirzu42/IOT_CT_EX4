...
mono_hlsb_test::mono_hlsb_test(uint16_t width, uint16_t height) : mono_horiz(width, height, width) {
}

void mono_hlsb_test::show() {
    std::ofstream header_file("output.h");
    uint8_t *data = buffer.get();

    header_file << "const unsigned char binary_data[] = {\n";
    header_file << "// font edit begin : monohlsb : " << width << " : " << height << " : " << stride << "\n";
